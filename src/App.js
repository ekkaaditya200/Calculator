...
function App() {
  ...
  const [history, setHistory] = useState(JSON.parse(localStorage.getItem("history")) || false || []);
  ...
  const handleKeyPress = (keyCode, key) => {
    ...
    if (numbers.includes(key)) {
      ...
      const lastChar = expression.slice(-1); //give the last character
      ...
      setExpression(expression.slice(0, -1))//Give expression from 0 to n-1 i.e erase the last character
    }
    else if (keyCode === 13) {
      if (!expression) return;
  ...
  return (
    <div className="app"
      ...
      <div className='app_calculator'>
        ...
        <Keypads handleKeyPress={handleKeyPress} />
